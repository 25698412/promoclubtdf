...
export default function RegisterPage() {
  ...
  const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', password: '', phone: '', city: '' });
  ...
  const validateStep1 = () => {
    if (!formData.firstName || !formData.lastName) { setError('Por favor completá tu nombre y apellido'); return false; }
    if (!formData.email || !formData.email.includes('@')) { setError('Por favor ingresá un email válido'); return false; }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (authError) {
      ...
      if (profileError) { setError(profileError.message); } else { router.push('/dashboard'); }
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-500 via-primary-600 to-secondary-500 flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md animate-scale-in">
        ...
        <div className="flex items-center justify-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${step >= 2 ? 'bg-[#F58220] text-white' : 'bg-white/20 text-white/60'}`}>2</div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            {step === 1 ? (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <Input label="Nombre" name="firstName" type="text" value={formData.firstName} onChange={handleChange} placeholder="Juan" leftIcon={<FiUser size={18} />} required />
                  <Input label="Apellido" name="lastName" type="text" value={formData.lastName} onChange={handleChange} placeholder="Pérez" leftIcon={<FiUser size={18} />} required />
                </div>
                <Input label="Email" name="email" type="email" value={formData.email} onChange={handleChange} placeholder="tu@email.com" leftIcon={<FiMail size={18} />} required />
              ...
              <>
                ...
                <Input label="Teléfono (opcional)" name="phone" type="tel" value={formData.phone} onChange={handleChange} placeholder="+54 9 2901 123456" leftIcon={<FiPhone size={18} />} />
                <Input label="Ciudad (opcional)" name="city" type="text" value={formData.city} onChange={handleChange} placeholder="Ushuaia" leftIcon={<FiMapPin size={18} />} />
                <button type="button" onClick={() => setStep(1)} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">← Volver al paso anterior</button>
                <button type="submit" disabled={loading}
                  ...
                  style={{ background: 'linear-gradient(135deg, #F58220 0%, #e0711a 100%)', boxShadow: '0 4px 15px rgba(245, 130, 32, 0.35)' }}>
                  {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Crear Cuenta'}
                </button>
              </>
            )}
          </form>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm"><span className="bg-white px-4 text-gray-400">o registrate con</span></div>
          </div>

          <button onClick={handleGoogleSignUp} className="w-full flex items-center justify-center gap-3 px-4 py-3.5 border-2 border-gray-200 rounded-xl hover:border-gray-300 hover:bg-gray-50 transition-all duration-200 group">
        ...
        <p className="text-center mt-8 text-white/80">
          ¿Ya tenés cuenta?{' '}
          <Link href="/login" className="text-white font-semibold hover:text-[#F58220] transition-colors">Iniciar Sesión</Link>
        </p>
      </div>
    </div>
  );
