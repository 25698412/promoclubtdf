...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    // MODO DEMO: Acceso público sin autenticación
    const loadDemoData = async () => {
      // Intentar cargar datos reales de Supabase si están disponibles
      try {
        ...
        // Datos de demo si falla Supabase
        setStats({
          totalBusinesses: 12,
          totalPromotions: 48,
          totalUsers: 1250,
  ...
  const topPromotions = [
    { id: 1, name: '50% OFF en Hamburguesas', business: 'Burger House', redemptions: 156, status: 'active' },
    { id: 2, name: '30% OFF en Zapatillas', business: 'Sport Center', redemptions: 98, status: 'active' },
    { id: 3, name: '40% OFF en Celulares', business: 'Tech Store', redemptions: 87, status: 'active' },
    { id: 4, name: '20% OFF en Libros', business: 'Librería Central', redemptions: 65, status: 'active' },
  ];
            ...
            <div className="mb-8 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Link href="/admin/businesses/new">
                  <Card className="hover-lift cursor-pointer group">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-primary-50 rounded-lg flex items-center justify-center text-primary-500 group-hover:bg-primary-500 group-hover:text-white transition-colors">
                        <FiPlus size={18} />
                      </div>
                      <div>
                        <p className="font-medium text-gray-900">Nuevo Local</p>
                        <p className="text-sm text-gray-500">Registrar comercio</p>
                      </div>
                    </div>
                  </Card>
                </Link>
                <Link href="/admin/promotions/new">
                  <Card className="hover-lift cursor-pointer group">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-accent-50 rounded-lg flex items-center justify-center text-accent-500 group-hover:bg-accent-500 group-hover:text-white transition-colors">
                        <FiPlus size={18} />
                      </div>
                      <div>
                        <p className="font-medium text-gray-900">Nueva Promoción</p>
                        <p className="text-sm text-gray-500">Crear oferta</p>
                      </div>
                    </div>
                  </Card>
                </Link>
                <Link href="/admin/banners/new">
                  <Card className="hover-lift cursor-pointer group">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-success-50 rounded-lg flex items-center justify-center text-success group-hover:bg-success group-hover:text-white transition-colors">
                        <FiPlus size={18} />
                      </div>
                      <div>
                        <p className="font-medium text-gray-900">Nuevo Banner</p>
                        <p className="text-sm text-gray-500">Crear banner</p>
                      </div>
                    </div>
                  </Card>
                </Link>
                    ...
                    {recentActivity.map((activity) => (
                      <div key={activity.id} className="p-4 flex items-center gap-4 hover:bg-gray-50 transition-colors">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                          ...
                          activity.status === 'pending' ? 'bg-error-50 text-error' :
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          >
                            {activity.status === 'success' ? 'Completado' :
                             activity.status === 'warning' ? 'Canjeado' :
              ...
              <div className="animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
                <Card padding="none">
                  ...
                  <div className="divide-y divide-gray-100">
                    {topPromotions.map((promo, index) => (
                      <div key={promo.id} className="p-4 flex items-center gap-3 hover:bg-gray-50 transition-colors">
                        <div className="w-8 h-8 bg-primary-50 rounded-lg flex items-center justify-center text-primary-500 font-bold text-sm flex-shrink-0">
                          {index + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                  ...
                  <div className="p-4 border-t border-gray-100">
                    <Link href="/admin/promotions" className="text-sm text-accent-500 hover:text-accent-600 font-medium">
